"""RAG ingestion pipeline: parse PDFs, chunk, embed with vLLM, insert into Milvus.
...
3-stage Ray Data pipeline submitted as a RayJob to an existing RayCluster:
  ...
  Stage 2: vLLM embedding          -- embed via ray.data.llm processor (GPU)
...
Execution model:
  ...
  pole. The vLLM embedding stage runs on GPU workers via vLLMEngineProcessorConfig.
...
class DoclingChunkActor:
    ...
    def __init__(self):
        ...
        self.chunker = HybridChunker(
            tokenizer=VLLM_MODEL_SOURCE, max_tokens=CHUNK_MAX_TOKENS
        )
    ...
    def __call__(self, batch: Dict[str, List]) -> Dict[str, List]:
        ...
        for file_path in batch["path"]:
            ...
            try:
                ...
                if file_size < 100:
                    logger.warning("File too small, skipping: %s", fname)
                    continue
...
def _run_pipeline(ds) -> Dict[str, Any]:
    """Run the 3-stage streaming pipeline: chunk -> embed (vLLM) -> insert.

    Stage 1 uses map_batches with CPU actors for Docling parsing/chunking.
    Stage 2 uses the Ray Data LLM processor with vLLMEngineProcessorConfig
    ...
    print(
        ...
        f"vllm_embed=concurrency={VLLM_CONCURRENCY} (GPU), "
...
def _build_metrics(
    ...
    metrics = {
        "embedding_model": str(VLLM_MODEL_SOURCE),
