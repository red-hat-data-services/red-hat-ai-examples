...
LoRA on Ray supports two execution modes:

- **Single GPU** ([`lora_sft-ray.ipynb`](./lora_sft-ray.ipynb)): Direct invocation of `lora_sft()` on the head pod. Uses a single GPU.
- **Multi-GPU Distributed** ([`lora_sft-ray-multinode.ipynb`](./lora_sft-ray-multinode.ipynb)): Distributed across multiple Ray workers via `TorchTrainer`.
...
The notebook submits a `RayJob` via CodeFlare SDK. The SDK creates a short-lived RayCluster (head-only), runs the Training Hub `lora_sft()` entrypoint, and tears everything down on completion.

### Multi-GPU Distributed (TorchTrainer)

The notebook submits a `RayJob` that uses Ray Train's `TorchTrainer` to distribute training across multiple workers. `TorchTrainer` sets up the distributed environment across the cluster, enabling data-parallel LoRA training.
...
### Ray Cluster Requirements (Single GPU)

| Component | GPU | GPU Type | CPU | Memory |
|-----------|-----|----------|-----|--------|
| Head pod | 1× GPU | NVIDIA L40S/A100 (48GB+ VRAM) | 4 cores | 64Gi |

### Ray Cluster Requirements (Multi-GPU Distributed)

The default configuration uses 1 head node + 1 worker node, each with 1 GPU (2 GPUs total).

| Component | GPU | GPU Type | CPU | Memory |
|-----------|-----|----------|-----|--------|
| Head pod | 1× GPU | NVIDIA L40S/A100 (48GB+ VRAM) | 4 cores | 64Gi |
| Worker pod (×1) | 1× GPU | NVIDIA L40S/A100 (48GB+ VRAM) | 4 cores | 64Gi |
...
- Navigate to the `examples/fine-tuning/lora_ray` directory and open [`lora_sft-ray.ipynb`](./lora_sft-ray.ipynb) for single GPU or [`lora_sft-ray-multinode.ipynb`](./lora_sft-ray-multinode.ipynb) for multi-GPU distributed.
